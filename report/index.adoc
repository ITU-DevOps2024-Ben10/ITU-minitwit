...
=== System Monitoring
This project utilizes a collaboration between Grafana and Prometheus.
The flow of the monotoring:

* ASP.Net exposes some perfomance metrics by default
* Custom metrics that are exposed:
...
* Prometheus is configured through a YML script, to scrape "/metrics" endpoints at port 8080 and 8081 every 15 seconds this includes both the custom and the default metrics related to the web application.
* Grafana queries and aggregates the data from the Prometheus Server, and displays them in a custom Dashboard created by the team. 

*MISSING FIGURE OF MONITORING CONFIGURATION*
...
* If used without careful inspector of the provided code, the LLM is likely to introduce bugs into the application. This is due to the fact,
...
There are multiple options to remove this threat; any service providing 2FA for secret repositories, like Docker vaults.
...
There are many options as to how to improve on this threat, a minimum would be to have physical copy of the state of the database on one or more hard drives. DigitalOcean has a Collaboration with SnapShooter, a service that enables virtual backups of databases, that would integrate nicely into the project. 
