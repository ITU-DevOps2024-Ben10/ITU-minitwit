...
As seen in figure 2, the source code consists of the three main pillars; Core, Infrastructure, and Web. This coincides with the Onion architecture with the Core being the innermost layer, the domain model, the next layer, infrastructure, being the business logic, and the outermost layer, Web, being the user interface. These layers each only depend inwards which has been beneficial for the project as it enforces a clear separation of concerns, which allows for easier management of dependencies and testing.
