...
The upgrade strategy of the system employs the default rolling update mechanism provided by Docker Swarm. This strategy updates one task at a time, ensuring that the application remains available during the upgrade process. The alternative Upgrade Strategy option was the Blue-Green deployment strategy, where two identical environments (blue and green) run simultaneously and traffic is switched to the new version only after full deployment, rolling updates offer a more gradual approach. While Blue-Green can provide near-zero downtime and an easy rollback mechanism, it requires double the resources during deployment. In contrast, rolling updates consume fewer resources and offer continuous application availability, albeit with a slightly higher risk of partial downtime during the transition. <<ref_szulik>>
