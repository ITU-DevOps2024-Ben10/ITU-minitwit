...
public class ReactionRepositoryTest
{
    ...
    public async Task AddReaction_ShouldAddReactionToCheep(ReactionType reactionType)
    {
        ...
        Author authorDto = new Author { UserName = "TestAuthor1", Email = "mock1@email.com" };
        Author authorDto1 = new Author { UserName = "TestAuthor2", Email = "mock2@email.com" };
        ...
        //Act
        await _ReactionRepository.AddReaction(reactionType, cheepDto.CheepId, authorDto.Id);
        ...
        Assert.Equal(
            authorDto.Id,
            db.Reactions.FirstOrDefault(reaction => reaction.CheepId == cheepDto.CheepId)!.AuthorId
        );
        Assert.Equal(
            reactionType,
            db.Reactions.FirstOrDefault(reaction =>
                reaction.CheepId == cheepDto.CheepId
            )!.ReactionType
        );
    ...
    public async Task RemoveReaction_ShouldAddRemoveReactionFromCheep(ReactionType reactionType)
    {
        ...
        //Act
        await _ReactionRepository.RemoveReaction(reactionType, cheepDto.CheepId, authorDto.Id);
    ...
    public async Task GetReactionCount_ShouldReturnTheCorrectAmountOfReactions()
    {
        ...
        Assert.Equal(
            1,
            await _ReactionRepository.GetReactionCount(cheepDto.CheepId, ReactionType.Like)
        );
        Assert.Equal(
            2,
            await _ReactionRepository.GetReactionCount(cheepDto.CheepId, ReactionType.Dislike)
        );
        Assert.Equal(
            1,
            await _ReactionRepository.GetReactionCount(cheepDto.CheepId, ReactionType.Love)
        );
    }
    ...
    public async void HasUserReacted_ShouldReturnTrueWhenUserHasReacted()
    {
        ...
        //Act
        bool hasReacted = await _ReactionRepository.HasUserReactedAsync(
            cheepDto.CheepId,
            authorDto.Id
        );
