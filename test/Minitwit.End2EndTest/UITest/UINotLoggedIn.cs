...
public class UiNotLoggedIn : PageTest
{
    ...
    public async Task CheckPublicTimeline()
    {
        ...
        //Check Navbar
        await Expect(_page.GetByRole(AriaRole.Link, new() { Name = "Public Timeline" }))
            .ToBeVisibleAsync();
        await Expect(_page.GetByRole(AriaRole.Link, new() { Name = "Register" }))
            .ToBeVisibleAsync();
        await Expect(_page.GetByRole(AriaRole.Link, new() { Name = "Login" })).ToBeVisibleAsync();

        //Check Public Timeline
        await Expect(_page.GetByRole(AriaRole.Heading, new() { Name = "Public Timeline" }))
            .ToBeVisibleAsync();
    ...
    public async Task CheckRegister()
    {
        ...
        await Expect(_page.GetByRole(AriaRole.Heading, new() { Name = "Register", Exact = true }))
            .ToBeVisibleAsync();
        await Expect(_page.GetByRole(AriaRole.Heading, new() { Name = "Create a new account." }))
            .ToBeVisibleAsync();
        await Expect(_page.GetByText("Username")).ToBeVisibleAsync();
        await Expect(_page.Locator("#registerForm div").Filter(new() { HasText = "Email" }))
            .ToBeVisibleAsync();
    ...
    public async Task CheckLogin()
    {
        ...
        await Expect(_page.GetByRole(AriaRole.Link, new() { Name = "Login" })).ToBeVisibleAsync();
        await Expect(_page.GetByRole(AriaRole.Heading, new() { Name = "Log in", Exact = true }))
            .ToBeVisibleAsync();
        await Expect(
                _page.GetByRole(AriaRole.Heading, new() { Name = "Use a local account to log in." })
            )
            .ToBeVisibleAsync();
        await Expect(_page.GetByText("Username")).ToBeVisibleAsync();
        await Expect(_page.GetByText("Password", new() { Exact = true })).ToBeVisibleAsync();
